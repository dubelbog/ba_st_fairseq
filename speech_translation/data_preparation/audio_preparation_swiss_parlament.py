...
def manifest_preparation(manifest, track, data, tgt_text, track_path):
    ...
    fairseq.examples.speech_to_text.extract_fbank_features(waveform, sample_rate, feature_root / f"{utt_id}.npy")
...
def gen_voc(train_text, spm_filename_prefix):
    ...
    print(f.name)
    fairseq.examples.speech_to_text.gen_vocab(
        Path(f.name),
...
def preparation():
    ...
    split = "dev"
    task = "st_ch_de"
    fairseq.examples.speech_to_text.save_df_to_tsv(df, Path("../data/sound") / f"{split}_{task}.tsv")
    spm_filename_prefix = f"spm_char_{task}"
    # Generate config YAML
    fairseq.examples.speech_to_text.gen_config_yaml(
        Path("../data/sound"),
